    ...
    <div className="min-h-screen flex flex-col bg-slate-100">
      ...
      <header
        className={`sticky top-0 z-10 transition-all duration-500 ${
          scrolled
            ? "shadow-md py-1.5 bg-white/95 backdrop-blur-sm"
            : "py-2 bg-white"
        } border-b border-slate-200`}
      >
        <div className="w-full max-w-screen-xl mx-auto px-1 md:px-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {navItems.map((item) => (
                    <button
                      ...
                      className={`group flex items-center gap-1.5 px-2.5 py-1 rounded-full transition-all duration-300 text-xs font-medium ${
                        item.id === activePage
                          ? item.activeColor
                          : item.baseColor
                      } ${item.id !== activePage ? item.hoverColor : ""}`}
                    >
                      <span>{item.icon}</span>
        ...
        >
          <div className="w-full max-w-screen-xl mx-auto px-1 py-2 space-y-1.5">
            {currentUser && (
              <>
                ...
                <div className="flex flex-col gap-1.5">
                  {navItems.map((item) => (
                    <button
                      ...
                      className={`flex items-center justify-between p-2 rounded-lg transition-all duration-300 text-xs ${
                        item.id === activePage
                          ? item.activeColor
                          : item.baseColor
                      } ${item.id !== activePage ? item.hoverColor : ""}`}
      ...
      {/* Main Content */}
      <main className="flex-1 w-full max-w-screen-xl mx-auto px-1 md:px-4 py-1 md:py-4">
        <PageWrapper>
          <div className="bg-white rounded-lg shadow-sm p-1 md:p-4">
            <Outlet />
          </div>
        </PageWrapper>
      </main>
      ...
      <footer className="bg-white border-t border-slate-200 py-2">
        <div className="w-full max-w-screen-xl mx-auto px-1 md:px-4 text-center">
